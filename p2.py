...
if __name__ == '__main__':
    ...
    img_name = r"C052-001.tif"  # 图像的路径
    #img = cv2.imread(img_name, cv2.IMREAD_GRAYSCALE)    # 读入图像，cv2.IMREAD_GRAYSCALE:以灰度模式读入图像
    #img = cv2.imread(img_name, -1)    # 读入图像，cv2.IMREAD_GRAYSCALE:以灰度模式读入图像
    img = cv2.imread(img_name, -1)*255.0    # lsm
    ...
    cv2.imwrite("Basic3.tif", Basic_img)
    ...
    cv2.imwrite("Final3.tif", Final_img)
