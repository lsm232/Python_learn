...
class ResNet(nn.Module):
    #对于resnet18,34  block是Basicblock；对于resnet50,101 block是Bottleneck
    ...
    def _make_layer(self,block,channels,blocks_num,stride=1):
        ...
        if stride!=1 or self.in_channels!=block.expansion*channels:
            downsampple=nn.Sequential(
                nn.Conv2d(self.in_channels,channels*block.expansion,kernel_size=3,stride=stride,bias=False),
                nn.BatchNorm2d(channels*block.expansion),
            )
        layers=[]
        layers.append(block(self.in_channels,channels,downsampple=downsampple,stride=stride))
